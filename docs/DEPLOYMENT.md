...
- **Sin CORS** — solo para consumo desde el mismo dominio
...
- CORS restringido por defecto (ningún origen permitido); se habilita mediante la variable de entorno `CORS_ALLOWED_ORIGINS` (ver [CORS en producción](#cors-en-producción))
...
`src/api_server.py` restringe los orígenes CORS por defecto (ningún origen está permitido a menos que se configure explícitamente). Para habilitar el acceso desde un cliente en otro dominio, establece la variable de entorno `CORS_ALLOWED_ORIGINS` con una lista separada por comas de orígenes permitidos:

```bash
# Un solo origen
CORS_ALLOWED_ORIGINS=https://tu-app-cliente.com uvicorn src.api_server:app --host 127.0.0.1 --port 8081
```

En el archivo de entorno del servicio:

```env
CORS_ALLOWED_ORIGINS=https://tu-app-cliente.com,https://otro-cliente.com
```

Si la API solo la consume tu propio dashboard (mismo dominio), no es necesario configurar esta variable.
...
| `CORS_ALLOWED_ORIGINS` | Orígenes CORS permitidos para `api_server.py` (coma-separados) | — |
