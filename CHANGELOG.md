...
- **Migraciones de Base de Datos con Alembic** — Schema management automático para `alert_history.db`. Ejecuta `alembic upgrade head` para aplicar migraciones pendientes. `sessions.db` sigue usando DDL raw con `IF NOT EXISTS`. (PR #XX)
